...
from mylib._misc import rectify_basename
...
class NHentaiKit:
    ...
    def get_gallery(self, url: str):
        ...
        # src_id = soup.find('div', id='cover').img['data-mylib'].rsplit('/', 2)[-2]
        ...
        all_thumb_url_l = [i.img['data-mylib'] for i in soup('a', class_='gallerythumb')]
...
class HentaiCafeKit:
    ...
    def get_chapters(self, entry_soup: BeautifulSoup, entry_uri: str) -> list:
        ...
        if chapters:  # more than one chapter
            cover_uri = entry_soup.find('div', class_='entry-thumb').img['mylib']
