    ...
    if "pixiv.net" in url:
        ...
        if args:
            ...
            if pq_arg.startswith("pq"):
                ...
                if (
                    any(s in url for s in sort_types)
                    or "/search?q=" in url
                    or "/search/?q=" in url
                ):
                    gldl_args = [
                ...
                if "/search?q=" in url and "sort=relevance" in url:
                    gldl_args = MultiList(
                        [
                            ...
                            for u in (url, url.replace("sort=relevance", "sort=top"))
                        ]
                    )
                else:
                    gldl_args = MultiList(
    ...
    for sort_and_k in site_settings["sort_tag_list"]:
        ...
        r.append(
            [
                *common_args,
                "--range",
                pq_value_to_range_value(pq_value, k_factor),
                url + sort_tag,
            ]
        )
    return r
...
def search_tags_in_filter(search_tags: str):
    ...
    if "FLAG_TAGS_IN_FILTER" not in os.environ:
    ...
    filter = " and ".join(
        [
            f"'{i.lstrip('-')}' {'not in' if i.startswith('-') else 'in'} tags"
            for i in filter_tags_l
        ]
    )
    return tags_s, filter
...
class BooruSite:
    ...
    def get_latest_post_id(self) -> int:
        ...
        o = p.stdout
        # print(o)
        m = re.search(r"\sid\s*(\d+)", o)
        if m:
            return int(m.group(1))
        elif p.returncode:
            return int(p.stderr.strip().split('id=')[-1])
        else:
            print(url)
            print(p)
            print(o)
            input()
            raise ValueError("cannot find latest post id")


    def multi_id_range_pq(self, pq_value: int) -> list[tuple[int, int]]:
        """
        Return a list of tuples (id, n) where n is pq_min * k and id is latest_id - minus_id.
        The list is generated by iterating over steps and breaking when latest_id <= minus_id.
        :param pq_value: int, the value of pq
        :return: list of tuples (id, n)
        """
        pq_n = int(pq_value)
        pq_min = pq_n // 10 or 1
        latest_id = self.get_latest_post_id()
        step_min = latest_id // 10000
        steps = {
            step_min: 1,
            step_min * 100: 2,
            step_min * 5000: 3,
...
def pq_site_arg_func(options, site_args, site_host, site_name, url, site_settings):
    ...
    gldl_args = [
        *GLDLCLIArgs(
            o=[*options, 'directory=["{search_tags!S:R  / /} {category}"]'],
        ),
        *site_args,
        url,
    ]
    ...
    if site_args:
        ...
        if pq_arg.startswith("pq"):
            ...
            if set(pq_value) <= set(string.digits + "-"):
                ...
                search_tags_s, search_filter = search_tags_in_filter(tags_s)
                if search_filter:
                    url = f"https://{site_host}{tag_path_prefix}{search_tags_s}"
                    head_args.add(filter=search_filter)
                ...
                the_path = pq_value[1:].strip(r'\/"').strip(r'\/"')
                override_base_dir, target_dir = os.path.split(the_path)
                search_tags_s_from_dir = fstk.sanitize_xu(
                    target_dir.split(site_name)[0].strip(), reverse=True
                )
                search_tags_s, search_filter = search_tags_in_filter(
                    search_tags_s_from_dir
                )
                url = f"https://{site_host}{tag_path_prefix}{search_tags_s}"
                head_args = GLDLCLIArgs(
                    *site_args,
...
def pop_tag_from_args(args):
    return fstk.sanitize_xu(
        re.sub(r"[\[\]]", "", args.pop(0)),
        reverse=True,
        unescape_html=False,
        decode_url=False,
        unify_white_space=False,
    )
...
def args2url(args):
    ...
    if first in ("pixiv", "p"):
        ...
        if x[:3] == "id=":
            post_id = x[3:]
            if post_id.isdigit():
                # 其实没用 因为新旧域名都不再支持 数字id
                url = f"https://idol.sankakucomplex.com/posts/{post_id}"
            else:
    ...
    if not args:
        ...
        for cmd in cmd_l:
            try:
                ...
                # print(p.args)
                error_code = p.wait()
                if error_code and pause_on_error:
                    need_pause = True
            except KeyboardInterrupt:
