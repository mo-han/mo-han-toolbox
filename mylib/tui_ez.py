...
class LinePrinter:
    def __init__(self, width: int = 0, output=sys.stdout):
        self.output = output
        self.width = tricks_ez.constrained(width, int, 'x > 0', enable_default=True, default=0)
...
def prompt_choose_number(header: str, choices, default_num: int = None, *, in_columns=True):
    ...
    if in_columns:
        choices_menu = text_base.list2col_str(choices_menu, get_terminal_width() - 2, sep=0)
    else:
